...
async def get_dashboard_metrics_controller(sort_by: str = None, order: str = "desc"):
    try:
        ...
        income_resp = (
            ...
            .select(
                "company_id, fiscal_year, profit_for_year, basic_eps, total_operating_income"
            )
            .execute()
        )
        balance_resp = (
            supabase.table("balance_sheet")
            .select("company_id, fiscal_year, total_equity, total_liabilities")
            .execute()
        )
        ...
        for company in companies:
            ...
            income_data = [
                i for i in income_statements if i["company_id"] == company_id
            ]
            ...
            latest_income = sorted(
                income_data, key=lambda x: x["fiscal_year"], reverse=True
            )[0]
            latest_balance = sorted(
                balance_data, key=lambda x: x["fiscal_year"], reverse=True
            )[0]
            ...
            dte_ratio = (
                round(liabilities / equity, 4) if liabilities and equity else None
            )
            profit_margin = (
                round(profit / latest_income["total_operating_income"], 4)
                if latest_income["total_operating_income"]
                else None
            )
        ...
        if sort_by and sort_by in [
            ...
            metrics = sorted(
                metrics,
                key=lambda x: (x[sort_by] is not None, x[sort_by]),
                reverse=(order == "desc"),
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to compute dashboard metrics"
        )
