...
const CashGraph = ({ tableData }: tabelDataProps) => {
  const [metrics] = useAtom(cashFlowMetricsAtom);
  const [cashMetrics, setCashMetrics] = useAtom(cashFlowGraphAtom);

  if (cashMetrics.length < 1) {
    return <></>;
  }
    ...
    <div className="flex flex-col gap-5 mt-5">
      <div className="border-r border-[#40404F] flex items-stretch z-0">
        <BarGraph
          ...
          metrics={metrics}
          map={fieldsMap}
          left={tableData.length < 6 ? 150 : 50}
          right={tableData.length < 6 ? 150 : 50}
          width={chartWidth}
        />
      </div>
